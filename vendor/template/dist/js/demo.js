...
(function ($) {
  ...
  setTimeout(function () {
    if (
      ...
      Number(localStorage.getItem("DerlyAPP:Demo:MessageShowed")) < Date.now()
    ) {
      localStorage.setItem(
        "DerlyAPP:Demo:MessageShowed",
        Date.now() + 15 * 60 * 1000
      );
      // eslint-disable-next-line no-alert
      alert(
        "Selamat datang di DerlyAPP!, \nthis file is only created for testing purposes!"
      );
    }
  }, 1000);
  ...
  $container.append('<h5>Customize DerlyAPP</h5><hr class="mb-2"/>');
